...
(function(){
    ...
    function getContainingBlock(el) {
        ...
        if(elPosition == "relative" || elPosition == "static" || !elPosition) return parent;
        while(parent != null && parent != docEl) {
...
(function() {
    ...
    function getScrollOffsets() {
        ...
        if("pageXOffset"in window)
            ...
            return{ left : docEl.scrollLeft, top : docEl.scrollTop };
        };
    ...
    function getViewportDimensions() {
        ...
        if(typeof document.width == "number") {
            f = function(){
                return{ width : Math.min(document.width, docEl.clientWidth), 
                    height : Math.min(document.height, docEl.clientHeight)}
            };
...
(function() {
    ...
    // Crawling up the tree. 
            ...
            var offsetLeft = el.offsetLeft, 
                ...
                defaultView = doc.defaultView,
                bcs = defaultView.getComputedStyle(body,''),
        ...
        // Main loop -----------------------------------------------------------------------
            ...
            // XXX Mozilla, Safari Opera: body margin is neverincluded in body offsetLeft/offsetTop.
            ...
            // it's probably using the AVK-CSSOM disaster.
...
// A closure for initializing load time constants.
  ...
  // TODO: add an optional commonAncestor paramenter to the below.
    /** 
     * @memberOf APE.dom
...
(function() {
    ...
    function removeClass(el, klass) {
        el.className = normalizeString(el.className.replace(getTokenizedExp(klass, "g")," "));
    }
    /** @param {HTMLElement} el  
    ...
    function getTokenizedExp(token, flag){
        var p = token + "$" + flag; 
        return (Exps[p] || (Exps[p] = new RegExp("(?:^|\\s)"+token+"(?:$|\\s)", flag)));
    }
    ...
    function getElementsByClassName(el, tagName, klass){
        var exp = getTokenizedExp(klass,""),
            ...
            i = 0,
            counter = 0,
            returnedCollection = Array(len); // initial load size.
        for(; i < len; i++){
            if(exp.test(collection[i].className))
                returnedCollection[counter++] = collection[i];
        }
        returnedCollection.length = counter; // trim array.
