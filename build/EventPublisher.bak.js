...
/**
 * @class <p>
 * steals the event handler and creates a callStack. 
 ...
 * Instead, add multiple beforeAdvise with: publisher.addBefore(fp, thisArg).add(fp2, thisArg);
 * </p><p>
 * Adding beforeEach and afterEach, to create advice for each callback would require modification 
...
APE.EventPublisher.cleanUp = function() {
    var type, publisherList, publisher, i = 0, len;
	for(type in this.Registry) {
		publisherList = this.Registry[type];
		for(len = publisherList.length; i < len; i++) {
