 ...
 * @param {Object} styleObject in the form of {color: "#030", fontSize : "12px"}
 * @param {ufloat} [duration] optional number of seconds.
 * @param {Function} [transition] optional funtion that takes a float [0-1] and returns a float [0-1]
...
APE.extend(APE.anim.StyleTransition, APE.anim.Animation, {
     ...
     * Runs the animation, getting the correct value  
     * from each ITransitionAdapter.
     * This run() method gets called by Animation.
     */
	...
	init : function(styleObject) {
    	...
    	for(prop in styleObject) {
        	...
        	if(!toValue) continue; // CSSStyleRule.
        	...
        	if(prop == "opacity" && !("opacity"in this.style)
                ...
                if(prop == 'clip' && (!fromValue || fromValue.indexOf("auto") != -1)) {
                    fromValue = "rect(0px " + el.offsetWidth + "px " + el.offsetHeight + "px 0px)";
                }
            	else {
        	...
        	adapter = TransitionAdapterFactory.fromValues(prop, fromValue, toValue);
 ...
 * @return {ITransitionAdapter} ITransitionAdapter for a specific type of style setting during run.
 * The {ITransitionAdapter} implements blendTo(rationaValue).
 * @class
...
APE.anim.TransitionAdapterFactory = {
	...
	fromValues : function(prop, fromValue, toValue) {
    	...
    	if(this.positiveLengthExp.test( prop )) {
        	return new this.PositiveLengthTransitionAdapter(prop, fromValue, toValue);
        }
    	...
    	if(prop == "fontWeight" && this.intExp.test( fromValue ) && this.intExp.test( toValue )) {
        	return new this.FontWeightTransitionAdapter(prop, fromValue, toValue);
        }
        if(prop == "visibility" && this.noVisibilityExp.test(fromValue) 
            || prop == "display" && fromValue == "none")
            return new this.ImmediateThresholdTransitionAdapter(prop, fromValue, toValue);
...
(function() {
	...
	var APE = window.APE,
        ColorRGB = APE.color && APE.color.ColorRGB,
	...
	function ColorTransitionAdapter(prop, fromValue, toValue) {
        if(!ColorRGB) ColorRGB = APE.color.ColorRGB;
	...
	APE.extend(ColorTransitionAdapter, TransitionAdapter, {
    	...
    	blendTo : function(rationalValue) {
        	var c = ColorRGB.blend(this.fromValue, this.toValue, rationalValue, this.currentValue);
        	return c.toString();
        }
    });

	function LengthTransitionAdapter(prop, fromValue, toValue) {
    	var lengthExp = APE.anim.TransitionAdapterFactory.lengthExp,
            fromValues = lengthExp.exec(fromValue),
            toValues = lengthExp.exec(toValue);
	...
	LengthTransitionAdapter.prototype.blendTo = function(rationalValue) {
        ...
        return ((this.fromValue * inverse) + (this.toValue * rationalValue)) + this.units;
    };
    ...
    /**@ignore
     * extends LengthTransitionAdapter
     */
	...
	PositiveLengthTransitionAdapter.prototype.blendTo = function(rationalValue) {
    	var inverse = 1-rationalValue,
            v = Math.max((this.fromValue * inverse) + (this.toValue * rationalValue), 0) + this.units;
    	return v;
    };
	...
	function ClipTransitionAdapter(prop, fromValue, toValue) {
    	...
    	var fromString = clipExp.exec(fromValue.replace(zeroToPxExp, "0px$1"))[1],
        	toString = clipExp.exec(toValue.replace(zeroToPxExp, "0px$1"))[1];
	...
	function OpacityTransitionAdapter(prop, fromValue, toValue) {
    	TransitionAdapter.call(this, prop, parseFloat(fromValue), parseFloat(toValue));
    }

    APE.extend(OpacityTransitionAdapter, TransitionAdapter);     
    OpacityTransitionAdapter.prototype.blendTo = function(rationalValue) {
        var inverse = 1-rationalValue,
            v = Math.max((this.fromValue * inverse) + (this.toValue * rationalValue), 0);
        return v;
    };
    ...
    /**@ignore
     * constructor FilterTransitionAdapter
     * performs adapter service using IE filters crap. 
     */
	APE.extend(FilterTransitionAdapter, TransitionAdapter);
    FilterTransitionAdapter.prototype.blendTo = function(rationalValue) {
        var inverse = 1-rationalValue,
            v = Math.abs((this.fromValue * inverse) + (this.toValue * rationalValue), 0);
	...
	function FontWeightTransitionAdapter(prop, fromValue, toValue) {
        TransitionAdapter.call(this, prop, parseInt(fromValue), parseInt(toValue));
    }
	...
	FontWeightTransitionAdapter.prototype.blendTo = function(rationalValue) {
        var inverse = 1-rationalValue,
            v = (((this.fromValue * inverse) + (this.toValue * rationalValue))/100 << 0) * 100;
        if(v < 100) return 100;
        if(v > 900) return 900;
        return v;
    };
    ...
    /**@ignore
     * constructor ThresholdTransitionAdapter
     * This is sort of a Null Object. It does implement blendTo,
     * setting the toValue at the very end.
     */
	...
	ImmediateThresholdTransitionAdapter.prototype.blendTo = function(rationalValue) {
        if(rationalValue == 0) {
            return this.fromValue;
        }
        return this.toValue;
