APE.namespace("shopping.widget");
/**
 * @constructor
 * @param {string} id the id of the list.
 * @param {number} [timeDuration] how much time to take to perform each scroll.
 */
shopping.widget.Carousel = function(id, timeDuration, vertical) {
    this.id = id;
...
shopping.widget.Carousel.prototype = {
    pos : 0,
    ...
    init : function() {
        var el = document.getElementById(this.id),
            ...
            Carousel = shopping.widget.Carousel;
        ...
        for(i = 0, len = items.length; i < len; i++) {
            item = items[i];
            if(!item) break;
            if(!("tagName" in item)) item.parentNode.removeChild(item);
        }
        ...
        if(prev) {
            addCallback(prev, "click", Carousel.handleButtonClick);
        }
        if(next) {
            addCallback(next, "click", Carousel.handleButtonClick);
        }
    },
    ...
    moveStart : function(isNext) {
        ...
        // If the carousel is not wide enough to scroll, exit.
        ...
        if(!isNext) {
            dom.removeClass(document.getElementById(this.id + "Next"), "carouselButton-disabled");
            if(this.newPos >= 0) {
                d = -this.pos;
                this.newPos = 0;
                dom.addClass(document.getElementById(this.id + "Prev"), "carouselButton-disabled");
            }
        } else {
            // disable the "Prev" button if we're at 0.
            if(this.pos === 0 && this.newPos ) {
                dom.removeClass(document.getElementById(this.id + "Prev"), "carouselButton-disabled");
            }
            // disable the "Next" button if we're at end.
            if(this.newPos <= this.frameSize - this.scrollDistance) {
                dom.addClass(document.getElementById(this.id + "Next"), "carouselButton-disabled");
        ...
        this.dx = d;
        this.timerId = window.setInterval("shopping.widget.Carousel.instances['"+this.id+"'].move()", 12);
    },
...
shopping.widget.Carousel.getById = APE.getById;

/**
 * onclick callback for Left/Right buttons.
 */
shopping.widget.Carousel.handleButtonClick = function() {
    var dir = this.id.match(/(\w+)(Next|Prev)$/),
        c = shopping.widget.Carousel.getById(dir[1]);
