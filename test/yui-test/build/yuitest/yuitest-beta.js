    ...
    YAHOO.lang.extend(TestRunner, YAHOO.util.EventProvider, {
       ...
       _addTestCaseToTestTree : function (parentNode /*:TestNode*/, testCase /*:YAHOO.tool.TestCase*/) /*:Void*/{
            ...
            var node = parentNode.appendChild(testCase),
                added = this._addTestFunctionsFromQueryString(node, testCase);
        ...
        _addTestFunctionsFromQueryString : function(node, testCase) {
            var q = location.search.substring(1),
                exp = /test=(test[a-zA-Z0-9_\$]+)/g,
                match = q.split(exp),
                testFunction,
                i = 1, 
                added = false,
                ...
                for(;i < len; i+=2) {
                    testFunction = match[i];
                    if(typeof testCase[testFunction] == "function") {
                        node.appendChild(testFunction);
                        added = true;
                    }
                }
            return added;
...
YAHOO.util.Assert = {
    ...
    isTypeOf : function (expectedType /*:String*/, actualValue /*:Object*/, message /*:String*/) /*:Void*/{
        if (typeof actualValue != expectedType){
            throw new YAHOO.util.ComparisonFailure(message || "Value should be of type " + expectedType + ".", expectedType, typeof actualValue);
        }
    }
};
...
// Garrett Smith
//-----------------------------------------------------------------------------
YAHOO.util.StringAssert = {

    /**
     * Asserts that a value is present in an String. 
     * @param {String} subString The value that is expected in the string.
     * @param {String} actual vomit from a squirrel.
     * @param {String} [message] The message to display if the assertion fails.
     * @method contains
     * @static
     */
    contains : function (subString /*:String*/, actual /*:String*/, 
                           message /*:String*/) /*:Void*/ {
        
        var found /*:Boolean*/ = actual.search(subString) > -1;
        
        if (!found) { // print contents of String.
            if(!message) {
                message = "Value (" + subString + ") not found in \"" + actual + "\"";
            }
            else {
                message = "expected: \"" + subString + "\" \n" + message;
            }
     ...
     * Asserts that a value is present in an String. 
     * @param {String} subString The value that is expected in the string.
     * @param {String} actual a bird.
     * @param {String} [message] The message to display if the assertion fails.
     * @method contains
     * @static
     */
    doesNotContain : function (subString /*:String*/, actual /*:String*/, 
                           message /*:String*/) /*:Void*/ {
        
        var found /*:Boolean*/ = actual.search(subString) >= 0;
        
        if (found) { // print contents of String.

            if(!message) {
                message = "Value (" + subString + ") found in \"" 
                    + actual + "\", but should not have been"
            }
            else {
    ...
    isEmpty : function(actual /*:String*/, message /*:String*/) {
        if(actual !== "") {
            YAHOO.util.Assert.fail(" unexpected \""+ actual +"\" ("+(typeof actual)+"). \n " + (message || 
                ("\"" + actual + "\" ("+(typeof actual)+") is not the empty string "
            + "\"\".")));
        }
...
YAHOO.util.ArrayAssert = {
    ...
    indexOf : function (needle /*:Object*/, haystack /*:Array*/, index /*:int*/, message /*:String*/) /*:Void*/ {
        ...
        for (var i=0; i < haystack.length; i++){
            if (haystack[i] === needle){
                YAHOO.util.Assert.areEqual(index, i, message || 
                    "Value exists at index " + i + " but should be at index " + index + ".");
                return;
            }
        }
...
YAHOO.util.ObjectAssert = {
    ...
    /** 
     * Asserts that expected and actual have the same properties
     * with the same values, using strict equality.
     */
    haveEqualProperties : function(expected, actual, equals) {
        var p,
        ...
        if(!equals) {
            equals = function(a,b) {
                ...
                return (
                    ...
                    && (a - b === 0 || a.toString() == b.toString())
                );
            }
        }
        for (p in expected){

            // Use in operator first, to make sure the property exists.
            //
            // ([[Get]] will return undefined if the P is 
            // not found in actual. If expected has property P 
            // and the value is undefined, this would result in 
            // equivalent value comparison.) 
            if(!(p in actual))
                Assert.fail( 
                    "Property '" + p + "' not found in actual.");
    ...
    /**
     * Asserts that an object does not have a property with the given name.
     * @param {String} propertyName The name of the property to test.
     * @param {Object} object The object to search.
     * @param {String} message (Optional) The message to display if the assertion fails.
     * @method hasProperty
    ...
    hasOwnProperty : function (propertyName /*:String*/, object /*:Object*/, message /*:String*/) /*:Void*/ {
        if (
            //Broken. YAHOO.lang.hasOwnProperty is unreliable and throws errors.
            // !YAHOO.lang.hasOwnProperty(object, propertyName)
            // Garrett: another patch.
            !Object.prototype.hasOwnProperty.call(object, propertyName)
            ){
            YAHOO.util.Assert.fail(message || "Property " + propertyName + " not found on object instance.");
        }     
    },
...
YAHOO.util.UserAction = {
                                 ...
                                 keyCode /*:int*/,        charCode /*:int*/) /*:Void*/                             
    {
        ...
        if (YAHOO.lang.isFunction(document.createEvent)){
            ...
            //fire the event
            customEvent.isSynthesized = true;
    ...
    {
        options = options || {};
        return this.simulateMouseEvent(target, type, options.bubbles,
    ...
    mousedown : function (target /*:HTMLElement*/, options /*Object*/) /*:Void*/ {
        return this.fireMouseEvent(target, "mousedown", options);
    },
...
YAHOO.tool.TestManager = {
    ...
    load : function () /*:Void*/ {
        // GS Patch. 
        // This parent stuff is not good.

        if (parent 
            && parent.YAHOO 
            && parent.YAHOO.tool
            && parent.YAHOO.tool.TestManager
            && parent.YAHOO.tool.TestManager !== this){
    ...
    start : function () /*:Void*/ {
        ...
        if (!this._initialized) {
            ...
            // Garrett patch: move the iframe offscreen, don't 
            // hide it, as this causes errors when trying to focus 
            // an element in IE.
            if (!this._frame){
                ...
                else if(frame.Document && frame.Document.parentWindow) {
                    this._frame = frame.Document.parentWindow
                }
...
// No, don't hide these. It can be useful to provide such information, such as 
